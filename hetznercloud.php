...
function hetznercloud_SuspendAccount(array $params)
{
    ...
    try {
        ...
        if (isset($data['action']['status']) && $data['action']['status'] === 'success') {
            logModuleCall('hetznercloud', 'SuspendAccount', $params, 'Success - Server ID: ' . $serverID . ' - Response: ' . $response);
            return 'success';
        } else {
            $error = 'Failed to suspend server: ' . (isset($data['error']['message']) ? $data['error']['message'] : $response);
